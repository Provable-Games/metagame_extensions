...
cat > "$DEPLOYMENT_FILE" << EOF
{
  ...
  "opus_troves_validator": {
    ...
    "features": [
      ...
      "Optional per-context banning when debt falls below threshold or exceeds quota (set bannable=1 in config)"
    ]
  }
}
EOF
...
echo "  Bannable: configured per-context via add_config (set last config field to 1 to enable)"
echo ""
...
echo "  config[N+4]: bannable (bool) - 1 = entries can be banned mid-tournament, 0 = no banning"
echo ""
...
echo "  # Config: asset_count=0 (wildcard), threshold=1 yin, value_per_entry=1 yin, max=50, bannable=1"
echo ""
...
echo "  # Config: asset_count=1, asset=STRK, threshold=10 yin, value_per_entry=0.5 yin, max=20, bannable=1"
echo ""
...
echo "  # Config: asset_count=2, assets=STRK+wstETH, threshold=5 yin, value_per_entry=2 yin, max=0 (unlimited), bannable=0 (no banning)"
echo ""
