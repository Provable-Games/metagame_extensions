...
if [ ! -f "target/dev/budokan_validators_OpusTrovesValidator.contract_class.json" ]; then
    print_error "OpusTrovesValidator contract build failed or contract file not found"
    print_error "Expected: target/dev/budokan_validators_OpusTrovesValidator.contract_class.json"
...
DECLARE_OUTPUT=$(sncast --profile $SNCAST_PROFILE --wait declare \
    ...
    if echo "$DECLARE_OUTPUT" | grep -q "already declared"; then
        print_warning "OpusTrovesValidator already declared"
        CLASS_HASH=$(echo "$DECLARE_OUTPUT" | grep -oE '0x[0-9a-fA-F]+' | head -1)
    else
        print_error "Failed to declare OpusTrovesValidator"
        echo "$DECLARE_OUTPUT"
        exit 1
    fi
}
