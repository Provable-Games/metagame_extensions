...
if [ ! -f "target/dev/metagame_extensions_presets_ERC20BalanceValidator.contract_class.json" ]; then
    print_error "ERC20BalanceValidator contract build failed or contract file not found"
    print_error "Expected: target/dev/metagame_extensions_presets_ERC20BalanceValidator.contract_class.json"
...
echo "  Config array: [token_address, min_threshold_low, min_threshold_high, max_threshold_low, max_threshold_high, value_per_entry_low, value_per_entry_high, max_entries, bannable]"
echo ""
...
echo "  # Trailing 1 = bannable (set to 0 for no banning)"
echo ""
