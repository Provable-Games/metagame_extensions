use metagame_extensions_adventurer_oracle::interface::{
    Comparator, Condition, IAdventurerOracleDispatcher, IAdventurerOracleDispatcherTrait,
    IMinigameObjectivesDetailsDispatcher, IMinigameObjectivesDetailsDispatcherTrait,
    IMinigameObjectivesDispatcher, IMinigameObjectivesDispatcherTrait, Metric,
};
...
fn config(metric: Metric, comparator: Comparator, target: u64, aux: u64) -> Condition {
    Condition { metric, comparator, target, aux }
}
...
fn test_evaluate_scalar_metrics() {
    ...
    assert!(
        oracle_lib::evaluate_condition(adv, bag, config(Metric::Xp, Comparator::AtLeast, 400, 0)),
        "xp",
    );
    assert!(
        oracle_lib::evaluate_condition(adv, bag, config(Metric::Level, Comparator::AtLeast, 20, 0)),
        "level",
    );
    assert!(
        oracle_lib::evaluate_condition(adv, bag, config(Metric::Gold, Comparator::AtLeast, 250, 0)),
        "gold",
    );
    assert!(
        oracle_lib::evaluate_condition(adv, bag, config(Metric::Health, Comparator::Equal, 80, 0)),
        "health",
    );
    assert!(
        oracle_lib::evaluate_condition(
            adv, bag, config(Metric::BeastHealth, Comparator::AtMost, 30, 0),
        ),
        "beast",
    );
    assert!(
    ...
    assert!(
        oracle_lib::evaluate_condition(
            adv, bag, config(Metric::Strength, Comparator::GreaterThan, 10, 0),
        ),
        "str",
    );
    assert!(
        oracle_lib::evaluate_condition(
            adv, bag, config(Metric::Dexterity, Comparator::Equal, 7, 0),
        ),
        "dex",
    );
    assert!(
        oracle_lib::evaluate_condition(adv, bag, config(Metric::Vitality, Comparator::Equal, 9, 0)),
        "vit",
    );
    assert!(
        oracle_lib::evaluate_condition(
            adv, bag, config(Metric::Intelligence, Comparator::Equal, 4, 0),
        ),
        "int",
    );
    assert!(
        oracle_lib::evaluate_condition(adv, bag, config(Metric::Wisdom, Comparator::Equal, 6, 0)),
        "wis",
    );
    assert!(
        oracle_lib::evaluate_condition(
            adv, bag, config(Metric::Charisma, Comparator::Equal, 11, 0),
        ),
        "cha",
    );
    assert!(
        oracle_lib::evaluate_condition(adv, bag, config(Metric::Luck, Comparator::AtLeast, 20, 0)),
        "luck",
    );

    // Negative case.
    assert!(
        !oracle_lib::evaluate_condition(adv, bag, config(Metric::Xp, Comparator::AtLeast, 401, 0)),
        "xp 400 !>= 401",
    );
}
...
fn test_evaluate_item_equipped() {
    ...
    assert!(
        oracle_lib::evaluate_condition(
            adv, bag, config(Metric::ItemEquipped, Comparator::Equal, 42, 0),
        ),
        "equipped 42",
    );
    assert!(
        !oracle_lib::evaluate_condition(
            adv, bag, config(Metric::ItemEquipped, Comparator::Equal, 7, 0),
        ),
        "not equipped 7",
    );
    ...
    assert!(
        oracle_lib::evaluate_condition(
            adv, bag, config(Metric::ItemHeldAnywhere, Comparator::Equal, 42, 0),
        ),
        "held 42",
    );
    ...
    assert!(
        !oracle_lib::evaluate_condition(
            adv, bag, config(Metric::ItemInBag, Comparator::Equal, 42, 0),
        ),
        "not in bag",
    );
}
...
fn test_evaluate_item_in_bag_and_held_anywhere() {
    ...
    assert!(
        oracle_lib::evaluate_condition(
            adv, bag, config(Metric::ItemInBag, Comparator::Equal, 77, 0),
        ),
        "in bag 77",
    );
    assert!(
        oracle_lib::evaluate_condition(
            adv, bag, config(Metric::ItemHeldAnywhere, Comparator::Equal, 77, 0),
        ),
        "held 77",
    );
    assert!(
        !oracle_lib::evaluate_condition(
            adv, bag, config(Metric::ItemEquipped, Comparator::Equal, 77, 0),
        ),
        "not equipped 77",
    );
...
fn test_evaluate_item_greatness() {
    ...
    assert!(
        oracle_lib::evaluate_condition(
            adv, bag, config(Metric::ItemGreatness, Comparator::AtLeast, 5, 15),
        ),
        "max greatness 15 >= 15",
    );
    assert!(
        !oracle_lib::evaluate_condition(
            adv, bag, config(Metric::ItemGreatness, Comparator::AtLeast, 5, 16),
        ),
        "max greatness 15 !>= 16",
    );
    ...
    assert!(
        !oracle_lib::evaluate_condition(
            adv, bag, config(Metric::ItemGreatness, Comparator::AtLeast, 9, 1),
        ),
        "absent item greatness 0",
    );
}
...
fn test_fuzz_xp_atleast_matches_direct_compare(xp: u16, target: u16) {
    ...
    assert!(oracle_lib::evaluate_condition(adv, empty_bag(), cfg) == expected, "fuzz xp atleast");
}
...
/// Create a single-condition objective scoped to settings_id 0, no item set.
fn create(oracle: IAdventurerOracleDispatcher, condition: Condition) -> u32 {
    create_full(oracle, 0, array![condition], Option::None)
}

/// Create an objective with an explicit settings id, conditions, and optional item set.
fn create_full(
    oracle: IAdventurerOracleDispatcher,
    settings_id: u32,
    conditions: Array<Condition>,
    items: Option<Array<u8>>,
) -> u32 {
    start_cheat_caller_address(oracle.contract_address, owner_addr());
    let id = oracle.create_objective("name", "desc", settings_id, conditions, items);
...
fn test_create_objective_increments_ids() {
    ...
    let stored = oracle.get_objective_conditions(1);
    assert!(stored.len() == 1, "one condition");
    assert!(*stored.at(0).target == 100, "stored target");
    assert!(*stored.at(0).metric == Metric::Xp, "stored metric");
    assert!(oracle.get_objective_settings_id(1) == 0, "settings id");
}
...
fn test_create_objective_only_owner() {
    ...
    oracle
        .create_objective(
            "n", "d", 0, array![config(Metric::Xp, Comparator::AtLeast, 1, 0)], Option::None,
        );
}
...
fn test_create_objective_unknown_settings() {
    ...
    create_full(oracle, 5, array![config(Metric::Xp, Comparator::AtLeast, 1, 0)], Option::None);
}
...
fn test_settings_id_mismatch_returns_false() {
    ...
    let id = create_full(
        oracle, 1, array![config(Metric::Xp, Comparator::AtLeast, 1, 0)], Option::None,
    );
    let objectives = IMinigameObjectivesDispatcher { contract_address: oracle.contract_address };
...
fn test_batch_helpers_and_details() {
    ...
    // Settings Id + (metric + comparator) for the single condition.
    assert!(d.objectives.len() == 3, "3 detail rows");
...
fn test_transfer_ownership() {
    ...
    let id = oracle
        .create_objective(
            "n", "d", 0, array![config(Metric::Gold, Comparator::AtLeast, 1, 0)], Option::None,
        );
    stop_cheat_caller_address(oracle.contract_address);
...
// ---------------------------------------------------------------------------
// Composite conditions + item sets (unified create_objective)
// ---------------------------------------------------------------------------

#[test]
fn test_composite_conditions_all_required() {
    let (mock, oracle) = setup();
    // One objective: level >= 5 AND gold >= 200.
    let id = create_full(
        oracle,
        0,
        array![
            config(Metric::Level, Comparator::AtLeast, 5, 0),
            config(Metric::Gold, Comparator::AtLeast, 200, 0),
        ],
        Option::None,
    );
    let objectives = IMinigameObjectivesDispatcher { contract_address: oracle.contract_address };
    ...
    // Level ok (xp 25 -> level 5) but gold short -> incomplete.
    let mut adv = base_adventurer();
    adv.xp = 25;
    adv.gold = 199;
    mock.set_assets(token_id, adv, empty_bag());
    assert!(!objectives.completed_objective(token_id, id), "gold short -> incomplete");

    // Both satisfied -> complete.
    adv.gold = 200;
    mock.set_assets(token_id, adv, empty_bag());
    assert!(objectives.completed_objective(token_id, id), "both -> complete");

    assert!(oracle.get_objective_conditions(id).len() == 2, "two conditions stored");
}

#[test]
fn test_item_set_hold_all() {
    let (mock, oracle) = setup();
    // Own the whole set {10, 20, 30} (held anywhere), plus a scalar condition.
    let id = create_full(
        oracle,
        0,
        array![config(Metric::Level, Comparator::AtLeast, 1, 0)],
        Option::Some(array![10_u8, 20, 30]),
    );
    let objectives = IMinigameObjectivesDispatcher { contract_address: oracle.contract_address };
    let token_id: felt252 = 701;
    mock.set_token_settings(token_id, 0);

    // Missing item 30 -> incomplete.
    let mut adv = base_adventurer();
    adv.equipment.weapon = item(10, 1);
    let mut bag = empty_bag();
    bag.item_1 = item(20, 1);
    mock.set_assets(token_id, adv, bag);
    assert!(!objectives.completed_objective(token_id, id), "missing item -> incomplete");

    // All three held -> complete.
    bag.item_2 = item(30, 1);
    mock.set_assets(token_id, adv, bag);
    assert!(objectives.completed_objective(token_id, id), "full set -> complete");

    assert!(oracle.get_objective_items(id) == array![10_u8, 20, 30], "items round-trip");
}

#[test]
fn test_items_only_objective() {
    let (mock, oracle) = setup();
    // No conditions — only an item requirement.
    let id = create_full(oracle, 0, array![], Option::Some(array![42_u8]));
    let objectives = IMinigameObjectivesDispatcher { contract_address: oracle.contract_address };
    let token_id: felt252 = 702;
    mock.set_token_settings(token_id, 0);

    let mut adv = base_adventurer();
    adv.equipment.ring = item(42, 1);
    mock.set_assets(token_id, adv, empty_bag());
    assert!(objectives.completed_objective(token_id, id), "holds the item -> complete");
}

#[test]
#[should_panic(expected: 'Oracle: objective is empty')]
fn test_empty_objective_rejected() {
    let (_mock, oracle) = setup();
    create_full(oracle, 0, array![], Option::None);
}

#[test]
#[should_panic(expected: 'Oracle: item id is zero')]
fn test_zero_item_id_rejected() {
    let (_mock, oracle) = setup();
    create_full(oracle, 0, array![], Option::Some(array![1_u8, 0, 3]));
}

#[test]
fn test_holds_all_items_pure() {
    let mut adv = base_adventurer();
    adv.equipment.weapon = item(10, 1);
    let mut bag = empty_bag();
    bag.item_1 = item(20, 1);
    assert!(oracle_lib::holds_all_items(adv, bag, array![10_u8, 20].span()), "has both");
    assert!(!oracle_lib::holds_all_items(adv, bag, array![10_u8, 99].span()), "missing 99");
    assert!(oracle_lib::holds_all_items(adv, bag, array![].span()), "empty vacuously true");
    assert!(!oracle_lib::holds_all_items(adv, bag, array![0_u8].span()), "zero id not held");
}
